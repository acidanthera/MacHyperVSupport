...
HyperVPCIRoot *HyperVPCIRoot::_instance;
...
bool HyperVPCIRoot::start(IOService *provider) {
  ...
  if (kernelVersion >= KernelVersion::SnowLeopard && kernelVersion <= KernelVersion::Sierra) {
    _instance           = this;
    _setConsoleInfoAddr = OSMemberFunctionCast(mach_vm_address_t, getPlatform(), &IOPlatformExpert::setConsoleInfo);
...
#if defined(__i386__)
    UInt64 patched[2] {0x25FF | ((_setConsoleInfoAddr + 8) << 16), (UInt32)wrapSetConsoleInfo};
...
IOReturn HyperVPCIRoot::wrapSetConsoleInfo(IOPlatformExpert *that, PE_Video *consoleInfo, unsigned int op) {
